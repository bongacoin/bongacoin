...
#if defined(NDEBUG)
# error "Bongacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0434a122123c3ccf556a0b50f58e01bcef37f754ba5d20ba1c8b2e07c46c608c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bongacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bongacoin Signed Message:\n";
...
{
    // Bongacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1401600 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1401600); // Bongacoin: 1401600 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 15 * 60; // Bongacoin: 15 minutes
static const int64 nTargetSpacing = 1.5 * 60; // Bongacoin: 90 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bongacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bongacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0434a122123c3ccf556a0b50f58e01bcef37f754ba5d20ba1c8b2e07c46c608c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "18/April/2018 Tesla suspends Model 3 production";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524045686;
        ...
        assert(block.hashMerkleRoot == uint256("0x80bb0a104cf1ebcba0b84749d77c9976d6adcd58edff4fb8d7188457cab97059"));
...
unsigned char pchMessageStart[4] = { 0xbd, 0xd1, 0xb4, 0xc0 }; // Bongacoin: increase each by adding 4 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BongacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BongacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BongacoinMiner(CWallet *pwallet)
{
    printf("BongacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bongacoin-miner");
    ...
    try { loop {
        ...
        printf("Running BongacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BongacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BongacoinMiner, pwallet));
}
